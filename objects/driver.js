...
export class Driver {
  constructor() {
    ...
    hair.name = "single hair 2bcloned"
    hair.geometry.applyMatrix(new THREE.Matrix4().makeTranslation(0, 2, 0));
    let hairs = new THREE.Object3D();
    hairs.name = "all sides hair"
    this.hairsTop = new THREE.Object3D();
    this.hairsTop.name = "moving hairs";
    for (let i = 0; i < 12; i++) {
      let h = hair.clone();
      let col = i % 3;
  ...
  updateHairs() {
    ...
    for (let i = 0; i < hairs.length; i++) {
      let h = hairs[i];
      h.scale.y = 0.75 + Math.cos( 0.8 * (this.angleHairs + (i / 4)) ) * 0.2;
    }
