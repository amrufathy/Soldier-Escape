...
function init() {
  ...
  unction handleKeyDown(keyEvent) {
    const leftLane = -1;
    const rightLane = 1;
...
function addWorld() {
  ...
  unction addWorldTrees() {
   ...
   for (let i = 0; i < numTrees; i += 1) {
    ...
    nction addTree(inPath, row, isLeft) {
    let newTree;
    if (inPath) {
   ...
   if (treeClock.getElapsedTime() > treeReleaseInterval) {
    ...
    distanceCounter += 1;
     if (!hasCollided) {
      ...
      oExplosionLogic();
        nder();
        obalRenderID = requestAnimationFrame(update); // request next update
        
      
    nction doTreeLogic() {
      et oneTree;
        nst treePos = new THREE.Vector3();
        nst treesToRemove = [];
      reesInPath.forEach((element, index) => {
  ...
  scoreText.innerHTML = score.toString();
   ...
   rollingGroundSphere.rotation.x = 0;
    ...
    gameOverFlag = true;
      ...
      ancelAnimationFrame(globalRenderID);
